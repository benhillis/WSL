...
void ValidateContainerName(LPCSTR Name)
{
    ...
    for (; Name[i] != '\0'; i++)
    {
        ...
        {
            THROW_HR_WITH_USER_ERROR(E_INVALIDARG, Localization::MessageWslcInvalidContainerName(Name));
        }
    }

    THROW_HR_WITH_USER_ERROR_IF(E_INVALIDARG, Localization::MessageWslcInvalidContainerName(Name), i == 0 || i > WSLC_MAX_CONTAINER_NAME_LENGTH);
}
...
HRESULT WSLCSession::PullImage(LPCSTR ImageUri, const WslcRegistryAuthInformation* RegistryAuthenticationInformation, IProgressCallback* ProgressCallback)
try
{
