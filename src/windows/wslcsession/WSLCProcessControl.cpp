...
using wsl::windows::service::wslc::DockerContainerProcessControl;
using wsl::windows::service::wslc::DockerExecProcessControl;
using wsl::windows::service::wslc::VMProcessControl;
using wsl::windows::service::wslc::WSLCProcessControl;

std::pair<WSLCProcessState, int> WSLCProcessControl::GetState() const
{
    if (m_exitEvent.is_signaled())
    {
        WI_ASSERT(m_exitedCode.has_value());
        return {WslcProcessStateExited, m_exitedCode.value()};
    }
    else
    {
        return {WslcProcessStateRunning, -1};
    }
}

const wil::unique_event& WSLCProcessControl::GetExitEvent() const
{
    return m_exitEvent;
}
