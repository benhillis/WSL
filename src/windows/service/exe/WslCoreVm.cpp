...
std::vector<char> WslCoreVm::ProcessVirtioFsRequest(_In_ gsl::span<gsl::byte> Request)
{
    ...
    auto buildResponse = [header](const std::wstring& tag, const std::wstring& subname, const std::wstring& source, HRESULT result) {
        // Respond to the guest with the aggregate tag, child subname (inside the aggregate's
        // synthetic root), and the canonicalized source the device maps.
    ...
    else if (header->MessageType == LxInitMessageRemountVirtioFsDevice)
    {
        ...
        const auto result = wil::ResultFromException([&]() {
            ...
            //
            // The aggregate remount protocol always appends the child SubnameOffset, and guest
            // init ships in lockstep with the service, so a well-formed message includes it. The
            // size and bounds checks below only guard against a malformed or truncated payload
            // (so a bogus offset is never dereferenced). A single aggregate tag is shared by every
            // child, so the subname is required to identify the specific share -- there is no
            // tag-only fallback.
            //
